...
float GenericPMTResponse::HitTimeSmearing(float Q) {
	...
	if(timingResolution < fTResMinimum) {
		timingResolution = fTResMinimum;
	}
...
void ResponseBoxandLine20inchHQE::Initialize(int seed, const string& pmtname) {
	...
	fhighcharge_param[0] = 2 * ksig_param[0] * ksig_param[1] * ksig_param[3] * sqrt(ksig_param[3]) *
	                       exp(-ksig_param[1] * ksig_param[3]);
	fhighcharge_param[1] =
	    ksig_param[0] *
	    ((1 - 2 * ksig_param[1] * ksig_param[3]) * exp(-ksig_param[1] * ksig_param[3]) + ksig_param[2]);
