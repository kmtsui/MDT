...
void WCRootDataPileUpSpill::AddInteraction(const WCRootDataNuInt* aNuInt, float offset_time, int bunch_id) {
	...
	if(aNuInt->GetInteractionType() == 0) {
		fPupSpill.NIDNuInt += 1;
	}
	else if(aNuInt->GetInteractionType() == 1) {
		fPupSpill.NBeamBkg += 1;
	}
}
...
void WCRootDataPileUpSpill::AddDigiHitsToMDT(MDTManager* mdt,
	...
	for(int i = 0; i < nAllDigiHits; i++) {
		WCSimRootCherenkovDigiHit* aHit = (WCSimRootCherenkovDigiHit*)allDigiHits->At(i);
