...
#ifdef HYBRIDWCSIM
			for(int k = 0; k < 3; k++) {
				th->SetPosition(k, aHitTime->GetPhotonEndPos(k));
			}
			for(int k = 0; k < 3; k++) {
				th->SetDirection(k, aHitTime->GetPhotonEndDir(k));
			}
			for(int k = 0; k < 3; k++) {
				th->SetStartDirection(k, aHitTime->GetPhotonStartDir(k));
			}
#endif
			th->SetStartTime(aHitTime->GetPhotonStartTime());
			for(int k = 0; k < 3; k++) {
				th->SetStartPosition(k, aHitTime->GetPhotonStartPos(k));
			}
			if(!pr->ApplyDE(th)) {
				continue;
			}
		...
		for(hc->Begin(); !hc->IsEnd(); hc->Next()) {
			...
			for(int i = 0; i < aPH->GetNDigiHits(); i++) {
				...
				if(t >= tWinLowEdge && t <= tWinUpEdge) {
					...
					if(tubeCount.count(tubeID) == 0) {
						doFill = true;
					}
					else {
						doFill = fMultDigiHits ? true : false;
					}
					...
					if(doFill) {
						...
						anEvent->AddCherenkovDigiHit(q, t, tubeID, mPMT_module_id, mPMT_pmt_id, true_pe_comp);
						nHits             += 1;
						sumQ              += q;
						nCount            += 1;
						tubeCount[tubeID] = 1;
...
#else
		aEvtOut
		    ->AddTrack(ipnu, flag, m, p, E, startvol, stopvol, dir, pdir, stop, start, parenttype, time, id);
#endif
...
void WCRootData::CopyTree(const char* filename, const char* treename, const vector<bool>& savelist) {
