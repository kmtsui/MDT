...
void WCRootDataNuInt::LoadFiles(const char* filename) {
	...
	std::cout << " Starting from: " << fFileList[fItrFile].first << " " << fFileList[fItrFile].second << " "
	          << fItrEntry << "/" << fNumEntries << std::endl;
}
...
bool WCRootDataNuInt::Next() {
	if(fItrEntry >= fNumEntries) {
		...
		std::cout << " Reading " << fFileList[fItrFile].first << " " << fFileList[fItrFile].second << " "
		          << fItrEntry << "/" << fNumEntries << std::endl;
	}
	this->GetEntry(fItrEntry);
	if(fHasFriend) {
		fFriendC->GetEntry(fItrEntry);
	}
	fItrEntry++;
...
int WCRootDataNuInt::GetFileIdNumber(const char* filename, const char* prefix) {
	...
	if(reco_total_size != total_size) {
		...
		std::cout << "  - Input file name is not formatted in: -----" << sPrefix << "XXXXX.root "
		          << std::endl;
