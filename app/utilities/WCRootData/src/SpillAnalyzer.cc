...
bool SpillAnalyzer::IsSingleIDIneractionAtBunch(const WCRootDataPileUpSpill* input,
                                                const int bunch_id,
                                                int& nuIntIdx) {
	nuIntIdx = -1;
	...
	for(int i = 0; i < pup->NNuIntTot; i++) {
		...
		if(pup->NuIntBunch[i] == bunch_id) {
			if(pup->NuIntType[i] == 0) {
				nIDNuInt += 1;
				nuIntIdx = i;
			}
			else if(pup->NuIntType[i] == 1) {
				nBeamBkg += 1;
			}
		}
	}

	if(!(EdepoODTot[bunch_id] < 50. && nIDNuInt == 1)) {
		return false;
	}
...
bool SpillAnalyzer::HasMichelEFromOtherInteractionAtBunch(const WCRootDataPileUpSpill* input,
	...
	for(int i = 0; i < nTracks; i++) {
		...
		if(TMath::Abs(ipnu) != 11) {
			continue;
		}

		Float_t time = aTrack->GetTime();
		if(time > time_low && time < time_up) {
			cout << " Found " << ipnu << " at t=" << time << " time_low:" << time_low
			     << " time_up: " << time_up << " TrackNuIntIdx: " << pup->TrackNuIntIdx[i]
			     << " nuIntIdx: " << nuIntIdx << endl;
			nSIEleFromOtherNuInt += 1;
		}
	}

	if(nSIEleFromOtherNuInt == 0) {
		return false;
	}
